...
- Fixed table cell text contrast in `MasterDataManagement.jsx` (`Classifications`, `Items`, and `Locations` tabs), `EmployeeTasksPage.jsx`, and `InternTasksPage.jsx` by replacing `text-white` on names/titles with `text-[#556070]`.
...
- Verified clean build execution (`npm --prefix client run build` passed in 2.97s with 0 errors).
