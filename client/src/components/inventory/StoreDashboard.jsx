        ...
        <Layout currentPage="store-dashboard">
            <div className="p-4 lg:px-12 pb-24">
                <div className="max-w-7xl mx-auto w-full">
                    {/* Header */}
                    <div className="mb-8">
                        <h1 className="text-3xl font-bold text-[#556070] tracking-tight mb-1">Store Operations</h1>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                        {statCards.map((card, idx) => (
                            <div key={idx} className="bg-white border border-slate-200 shadow-sm rounded-xl p-5 shadow-lg relative overflow-hidden group hover:border-amber-500/30 transition-all">
                                ...
                                <div className="space-y-3">
                                    {quickActions.map((action, idx) => (
                                        <button key={idx} onClick={() => navigate(action.path)}
                                            className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-border-dark hover:border-amber-500/30 transition-all group">
                                        ...
                                        {recentMovements.map((log, idx) => (
                                            <div key={idx} className="px-6 py-3 flex items-center justify-between hover:bg-background-dark/30 transition-colors">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div className="text-[#556070] text-sm font-medium">{log.item?.name || log.movementType?.replace(/_/g, ' ')}</div>
